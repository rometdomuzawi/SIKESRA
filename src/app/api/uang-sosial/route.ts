...
export async function GET(req: NextRequest) {
  ...
  const data = await db.uangSosial.findMany({
    where,
...
export async function POST(req: NextRequest) {
  ...
  if (body.bulk) {
    ...
    for (const w of semuaWarga) {
      const existing = await db.uangSosial.findUnique({
        where: { wargaId_bulan_tahun: { wargaId: w.id, bulan: Number(bulan), tahun: Number(tahun) } },
      })
      if (!existing) {
        const rec = await db.uangSosial.create({
  ...
  if (isLunas) {
    await db.riwayatPembayaran.create({
      data: {
        wargaId, jenis: 'SOSIAL', bulan: Number(bulan), tahun: Number(tahun),
...
export async function PUT(req: NextRequest) {
  ...
  const existing = await db.uangSosial.findUnique({ where: { id } })
  if (!existing) return NextResponse.json({ error: 'Data tidak ditemukan' }, { status: 404 })

  const isLunas = status === 'LUNAS'
  const updated = await db.uangSosial.update({
    where: { id },
    data: {
  ...
  if (status && status !== existing.status) {
    if (isLunas) {
      const existingRiwayat = await db.riwayatPembayaran.findFirst({
        where: { wargaId: existing.wargaId, jenis: 'SOSIAL', bulan: existing.bulan, tahun: existing.tahun },
      })
      if (!existingRiwayat) {
        await db.riwayatPembayaran.create({
          data: {
            wargaId: existing.wargaId, jenis: 'SOSIAL', bulan: existing.bulan, tahun: existing.tahun,
...
export async function DELETE(req: NextRequest) {
  ...
  if (existing) {
    ...
    await db.uangSosial.delete({ where: { id } })
  }
