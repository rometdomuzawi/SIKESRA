...
export async function GET(req: NextRequest) {
  ...
  // WARGA hanya bisa lihat datanya sendiri
  ...
  const data = await db.uangSampah.findMany({
    where,
...
export async function POST(req: NextRequest) {
  ...
  // Bulk generate untuk satu bulan/tahun, semua warga
  if (body.bulk) {
    ...
    for (const w of semuaWarga) {
      const existing = await db.uangSampah.findUnique({
        where: { wargaId_bulan_tahun: { wargaId: w.id, bulan: Number(bulan), tahun: Number(tahun) } },
      })
      if (!existing) {
        const rec = await db.uangSampah.create({
  ...
  if (existing) {
    return NextResponse.json({ error: 'Data iuran untuk bulan/warga ini sudah ada' }, { status: 400 })
  }
  ...
  // Jika lunas, masukkan ke riwayat
  if (isLunas) {
...
export async function PUT(req: NextRequest) {
  ...
  const existing = await db.uangSampah.findUnique({ where: { id } })
  if (!existing) return NextResponse.json({ error: 'Data tidak ditemukan' }, { status: 404 })

  const isLunas = status === 'LUNAS'
  const updated = await db.uangSampah.update({
    where: { id },
    data: {
  ...
  // Update riwayat jika status berubah
  if (status && status !== existing.status) {
    if (isLunas) {
      const existingRiwayat = await db.riwayatPembayaran.findFirst({
        where: { wargaId: existing.wargaId, jenis: 'SAMPAH', bulan: existing.bulan, tahun: existing.tahun },
      })
      if (!existingRiwayat) {
        await db.riwayatPembayaran.create({
          data: {
            ...
            jumlah: Number(jumlah || existing.jumlah),
            metode: metode || 'TUNAI',
            keterangan,
          },
        })
      ...
      // Jika berubah jadi BELUM_BAYAR, hapus dari riwayat
...
export async function DELETE(req: NextRequest) {
  ...
  if (existing) {
    ...
    await db.uangSampah.delete({ where: { id } })
  }
